...
var maxPoints = function(points) {
  ...
  for (let i = 0; i < points.length; i++) {
    let point = points[i];
    let result = resolve(hash, point);
    max = Math.max(max, result);
  }
  return max;
};

function resolve(hash, point) {
  ...
  if (hash[xRight + "-" + yRight] !== undefined) {
    diagonal1 = resolve(hash, [xRight, yRight]);
  } else if (hash[xRight + "-" + yDown] !== undefined) {
    diagonal2 = resolve(hash, [xRight, yDown]);
  } else if (hash[xRight + "-" + y] !== undefined) {
    horizontal = resolve(hash, [xRight, y]);
  } else if (hash[x + "-" + yRight] !== undefined) {
    vertical = resolve(hash, [x, yRight]);
  }
  let key = point[0] + "-" + point[1];
