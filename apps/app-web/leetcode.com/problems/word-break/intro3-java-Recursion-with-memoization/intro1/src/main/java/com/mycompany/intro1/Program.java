...
public class Program {
    ...
    public boolean wordBreak(String s, List<String> wordDict) {
        ...
        return word_Break(s, new HashSet(wordDict), 0, cache);
    }

    private boolean word_Break(String s, HashSet wordDict, int start, Boolean[] cache) {
        if (start == s.length()) {
            return true;
        }
        if (cache[start] != null) {
        ...
        for (int end = start + 1; end <= s.length(); end++) {
            ...
            if (wordDict.contains(substring)) {
                boolean result = word_Break(s, wordDict, end, cache);
                cache[start] = result;
                if (result) {
                    return true;
                }
